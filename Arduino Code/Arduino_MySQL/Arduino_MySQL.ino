...
byte mac_addr[] = { 0x90, 0xA2, 0xDA, 0x0F, 0x59, 0x6B };
IPAddress server_addr(127,0,0,1);  // Supply the IP of the MySQL *server* here

char user[] = "root";         // can be anything but the user must have
char password[] = "root";   // access rights to connect (host must permit it)
...
void do_query(const char *q) {
  const column_names *c; // pointer to column values
  const row_values *r;   // pointer to row values
